...
import numpy as np
import glob
from pyqtgraph import PlotWidget, mkPen  # Import PlotWidget from pyqtgraph
import pandas as pd
import wave
import xml.etree.ElementTree as ET
import os
...
class MainApp(QMainWindow):
    def __init__(self):
        ...
        self.setup_signals()  # Connect UI signals to functions
        self.initialize_playback()  # Initialize playback variables

        # Modes setup
        self.modes = ["Uniform Range", "Hybrid Sounds", "Song", "Wiener Filter"]
    ...
    def setup_signals(self):
        # Connect UI signals to respective functions
        self.ui.quit_app_button.clicked.connect(QApplication.quit)
    ...
    def toggle_scale_mode_2(self):
        ...
        if self.current_mode == "Hybrid Sounds" or self.current_mode == "Song" or self.current_mode == "Wiener Filter":
            self.update_audio_equalizer()

    def apply_mode(self):
        mode_config = {
            "Hybrid Sounds": self.configure_hybrid_sounds_mode,
            "Song": self.configure_vocals_mode,
    ...
    def configure_uniform_range_mode(self):
        ...
        self.load_signal_data(
            "Data/Rectangular/generated_signal_10.0Hz_60.0Hz_110.0Hz_160.0Hz_210.0Hz_260.0Hz_310.0Hz_360.0Hz_410.0Hz_460.0Hz.csv"
        )

    def configure_hybrid_sounds_mode(self):
        self.labels = ["Wolf", "Owl", "Birds", "80s sine synth", "Studio", "Worn Tape"]
    ...
    def audio_callback(self, outdata, frames, time, status):
        ...
        for slider, freq_range in zip(self.ui.equalizer_sliders[1:], self.frequency_ranges[1:]):
            ...
            fft_data[freq_mask] *= gain  # Apply gain to the selected frequency range

        # Perform Inverse Fourier Transform to get the modified audio
        adjusted_chunk = np.fft.irfft(fft_data).astype(np.float32)
    ...
    def update_audio_equalizer(self):
        ...
        # Apply gain for each frequency range
        for slider, freq_range in zip(self.ui.equalizer_sliders, self.frequency_ranges):
            gain = slider.value() / 50.0  # Normalize gain (50 = no change)
            freq_mask = (fft_freqs >= freq_range[0]) & (fft_freqs <= freq_range[1])
            fft_data[freq_mask] *= gain
        ...
        self.adjusted_audio_data = np.fft.irfft(fft_data)
    ...
    def upload_audio_signal_file(self):
        ...
        if file_path:
            self.reset_signal()
            self.current_file = file_path  # Set the current file
            self.load_audio_signal(file_path)
            self.is_toggle=False
            self.update_audio_equalizer()
        ...
        def callback():
            ...
            if self.original_time is not None and self.original_amplitude is not None:
                ...
                if self.current_mode == "Uniform Range":
                    ...
                    self.plot_wiener_graphs(self.original_time, self.original_amplitude)
                elif self.current_mode == "Musical Instrument" or self.current_mode == "Song":
                    self.update_audio_equalizer()
    ...
    def plot_signal_uniform(self, time, amplitude, fs=1000, threshold_factor=0.05):
        ...
        if not hasattr(self, "original_signal_plotted") or not self.original_signal_plotted:
            ...
            self.input_cine_graph.plot([], [], pen=pg.mkPen(color="#62c6dd", width=2))
        ...
        # Apply each slider value to adjust the corresponding frequency range
        for i, (low, high) in enumerate(self.frequency_ranges):
            slider_value = self.ui.equalizer_sliders[i].value() / 50  # Normalize slider to range around 1

            # Range adjustment for frequency bands
            band_indices = np.where((frequencies >= low) & (frequencies < high))[0]

            # Scale only the targeted band by the slider value
            adjusted_freq_data[band_indices] = freq_data[band_indices] * slider_value

        # Inverse Fourier Transform to get the adjusted signal back in the time domain
        adjusted_signal = irfft(adjusted_freq_data)
        ...
        self.output_cine_graph.plotItem.setLabel('left', 'Amplitude (Adjusted)')
    ...
    def noise_reduction(self):
        ...
        if self.current_file:
            ...
            if self.current_file.endswith('.wav'):
                ...
                self.audio_data, self.sampling_rate = librosa.load(self.current_file + '_wiener.wav', sr=None)
